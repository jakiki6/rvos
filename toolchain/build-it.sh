...
cd build-binutils
../binutils-2.36/configure --prefix=$PREFIX --target=$TARGET --disable-multilib --with-arch=rv64im --with-abi=lp64
make -j $(nproc)
make install
cd ..
...
mkdir build-gcc
cd build-gcc
../gcc-10.2.0/configure --prefix=$PREFIX --target=$TARGET --enable-languages=c,c++ --disable-multilib --with-arch=rv64im --with-abi=lp64
